...
namespace Autimecia.Modules
{
    internal static class Assets
    {
        ...
        // CHANGE THIS
        private const string assetbundleName = "autimeciabundle";
        ...
        internal static void LoadAssetBundle()
        {
            ...
            {
                using (var assetStream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Autimecia." + assetbundleName))
                {
        ...
        {
            using (Stream manifestResourceStream2 = Assembly.GetExecutingAssembly().GetManifestResourceStream("Autimecia.HenryBank.bnk"))
            {
        ...
        public static void ShaderConversion(AssetBundle assets)
        {
            ...
            {
                AutimeciaPlugin.ModLogger.LogInfo($"Replacing shader {material.shader.name} of material {material} with hopoo shader.");
                string name = material.shader.name.ToLowerInvariant();
                var replacementShader = Resources.Load<Shader>(shaderLookup[name]);
                if (replacementShader) { material.shader = replacementShader; }
                else { AutimeciaPlugin.ModLogger.LogError($"No replacement shader found."); }
            }
        }
